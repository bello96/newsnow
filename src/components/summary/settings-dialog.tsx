...
export function SettingsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (next.email.enabled) {
        body.llmApiKey = next.llm.apiKey
        body.llmBaseUrl = DEEPSEEK_BASE_URL
        body.llmModel = next.llm.model
      } else {
        body.llmApiKey = ""
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="text-xs op-60 mb-4 p-3 bg-primary/5 rounded">
          使用 DeepSeek 模型。API Key 仅保存在浏览器本地；开启定时邮件后，将上传到服务器以便定时调用。
        </div>
        ...
        <div className="mt-5 pt-3 border-t border-primary/15">
          <label className={labelCls}>收件邮箱（最多 {MAX_RECIPIENTS} 个 · 手动 / 定时发送共用）</label>
          <div className="flex flex-col gap-2">
            ...
            {email.toEmails.length === 0 && (
              <span className="text-xs op-50">点「添加邮箱」填写收件人，再到「立即分析」后即可手动发送</span>
            )}
          </div>
        </div>
        ...
        <div className="mt-5 pt-3 border-t border-primary/15">
          ...
          >
            ...
            <span className="ml-auto text-xs">
              {email.enabled ? <span className="color-primary">● 已开启</span> : <span className="op-50">○ 关闭</span>}
            </span>
          </label>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button type="button" onClick={onClose} className="px-3 py-1.5 rounded hover:bg-primary/10 text-sm">
            取消
          </button>
          <button
            type="button"
